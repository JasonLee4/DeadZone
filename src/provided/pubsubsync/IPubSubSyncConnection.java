...
public interface IPubSubSyncConnection {
	...
	public static IPubSubSyncManager getPubSubSyncManagerAt(ILogger logger, IRMIUtils rmiUtils, int stubPort, String ipAddr) throws RemoteException {
		...
		try {
			...
			IPubSubSyncManager discMgr = (IPubSubSyncManager) registry.lookup(PUBSUBSYNC_REGISTRY_BOUND_NAME);
			if( null == discMgr) {
				throw new RemoteException("No IPubSubSyncManager bound to \""+PUBSUBSYNC_REGISTRY_BOUND_NAME+"\" at IP address = "+ipAddr+":"+IRMI_Defs.REGISTRY_PORT);
			}
			else {
